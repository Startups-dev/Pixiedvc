...
`/pixie` stays inside the normal PixieDVC root layout, so it keeps the existing header, footer, affiliate capture, referral capture, and visitor analytics. Phase 5 does not modify the global support widget.
...
- Existing global support widget remains visible on `/pixie`.
