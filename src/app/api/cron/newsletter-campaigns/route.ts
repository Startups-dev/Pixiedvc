...
import { runScheduledNewsletterCampaigns, NEWSLETTER_CAMPAIGNS_AUTOMATION_KEY } from '@/lib/newsletter-campaign-scheduler';
...
async function acquireLock(
  ...
  const lockName = 'newsletter-campaigns';
...
async function handleNewsletterCampaigns(request: NextRequest) {
  ...
  const run = await startAutomationRun({
    client,
    automationKey: NEWSLETTER_CAMPAIGNS_AUTOMATION_KEY,
    startedAt: now,
    metadata: { dryRun },
  });
  ...
  try {
    ...
    await completeAutomationRun({
      ...
      metadata: {
        dryRun,
        now: result.now,
        campaigns_processed: result.campaignsProcessed,
        failed_campaigns: result.failed,
      },
    });
...
export async function GET(request: NextRequest) {
  return handleNewsletterCampaigns(request);
}
