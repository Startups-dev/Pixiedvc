...
export default async function GuestTestEntryPage({ params }: { params: { token: string } }) {
  const token = params.token.trim().toLowerCase();
  const invite = await getPrivateTestInviteByToken(token);

  if (!isInviteUsable(invite) || invite.flow_type !== "guest") {
    return (
      <main className="mx-auto max-w-3xl px-6 py-16">
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <h1 className="text-2xl font-semibold text-slate-900">Invalid Guest Test Link</h1>
          <p className="mt-3 text-sm text-slate-600">
            This guest testing link is invalid, expired, or belongs to a different test flow.
          </p>
          <div className="mt-6">
